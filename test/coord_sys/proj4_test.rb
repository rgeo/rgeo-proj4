...
class TestProj4 < Test::Unit::TestCase # :nodoc:
  ...
  def test_simple_mercator_transform
    ...
    assert_xy_close(project_merc(0, 0), RGeo::CoordSys::Proj4.transform_coords(geography_, projection_, 0, 0, nil))
    assert_xy_close(project_merc(0.01, 0.01), RGeo::CoordSys::Proj4.transform_coords(geography_, projection_, 0.01, 0.01, nil))
    assert_xy_close(project_merc(1, 1), RGeo::CoordSys::Proj4.transform_coords(geography_, projection_, 1, 1, nil))
    assert_xy_close(project_merc(-1, -1), RGeo::CoordSys::Proj4.transform_coords(geography_, projection_, -1, -1, nil))
    assert_xy_close(unproject_merc(0, 0), RGeo::CoordSys::Proj4.transform_coords(projection_, geography_, 0, 0, nil))
    assert_xy_close(unproject_merc(10_000, 10_000), RGeo::CoordSys::Proj4.transform_coords(projection_, geography_, 10_000, 10_000, nil))
    assert_xy_close(unproject_merc(-20_000_000, -20_000_000), RGeo::CoordSys::Proj4.transform_coords(projection_, geography_, -20_000_000, -20_000_000, nil))
  end
  ...
  def test_point_projection_cast
    ...
    assert_close_enough(-0.9393598527244420, geo_point_.x)
    assert_close_enough(51.5740106527552697, geo_point_.y)
  end
